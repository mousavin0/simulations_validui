...
for (i in 1:(length(nvec))) {
  for (j in 1:length(rhovec)) {
    simlow <- simulation_ui(
      FUN = generatingdata_ui_high_logistic, rep = rep, n = nvec[i], rho = rhovec[j],
      vecy = 0.6, vect = 0.3,
    ...
    #
    #
    # simlow=simulation_ui(FUN=generatingdata_ui_high_logistic,rep=rep,n=nvec[i],rho=rhovec[j],
    #                      vecy=0.5,vect=0.3,
    #                      trueValue=param[j], param="Y1",
...
#CHANGES AFTER REVISION
save.image("/Users/niloofarmoosavi/Library/CloudStorage/OneDrive-Personal/PhD/validui/code/final/Paper Two-final/scenarioFourRevisionLogistic.Rdata")
...
temp <- bias_scen4_refit_sigNotCor_ours[1:3,c(1,3,5)]
rownames(temp) <- nvec
